...
webcam_active = False
alarm_path = r"E:\content\Jarvis Audio.mp3"
...
def open_close_webcam(action):
    ...
    if cv2 is None:
        say("OpenCV is not installed. Webcam functionality is not available.")
        return
    if "open" in action:
        webcam_active = True
...
def detect_weather(city):
    ...
    image_path = r"E:\content\Screenshot 2025-02-01 204119.png"
...
def set_alarm(alarm_time, alarm_sound):
    ...
    try:
        ...
        # Play the alarm sound (using system beep since pygame is not available)
        print("Alarm ringing! Press 'q' to stop.")
        while True:
